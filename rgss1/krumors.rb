...
module KRumors
  class << self
    ...
    def grant_spoils(rumor)
      @rumors = @heroes[@group_pos].rumors
    ...
    def retrieve_spoils
      ...
      @rumor.spoils_max.times do |n|
        ...
        case item.kind
        when :item then $game_party.gain_item(item.id, item.amount)
        when :weapon then $game_party.gain_weapon(item.id, item.amount)
        when :armor then $game_party.gain_armor(item.id, item.amount)
    ...
    def recovery_time() @hurt_heroes ? @min * KRumorSetup::RECOVERY_TIME : 0 end
    def add_place(loc_id) @locations[loc_id] = @all_locations[loc_id].dup end
    def hero_id() @heroes[@group_pos].id end
  ...
  @min = Graphics.frame_rate * 60
  @all_locations = load_data('Data/KRumorsLocations.rxdata')
  @all_rumors = load_data('Data/KRumors.rxdata') rescue {}
  @npc_rumors = load_data('Data/KRumorsNPC.rxdata') rescue {}
  @rumor_opinions = load_data('Data/KRumorsOpinions.rxdata') rescue {}
  @book_titles = load_data('Data/KRumorsBookTitles.rxdata') rescue {}
  if $HIDDENCHEST
...
unless $HIDDENCHEST
  ...
  class Font
    def out_color=(new_color) @out_color = new_color end
  end
...
class Game_System
  alias kyon_krumors_gm_sys_up update
  def update
    kyon_krumors_gm_sys_up
    $game_party.rumor_heroes.each {|tr| tr.update_timer }
  end
end
...
class Game_Actor
  ...
  def update_timer() @timer -= 1 if @timer > 0 end
  def hplow?() @hp < maxhp / 4 end
...
class Game_Party
  ...
  def send_traveler(hero_id, locate_pos)
    ...
    if KRumors.hurt_heroes
      hero.hp -= hero.maxhp * KRumorSetup::HP_DMG_PERCENT / 100
    end
    @destinations[hero_id] = @rumor_places[place_id].name
    @reserves.delete_if{|h| h.id == hero_id }
    @travelers << hero
  ...
  def skip_travel(actor_id)
    traveler = @travelers.delete($game_actors[actor_id])
  ...
  def update_returners
    ...
    heroes = @travelers.select{|h| h.timer < 20 }
  ...
  def returners_become_idle
    heroes = @returners.select{|h| h.no_new_rumors }
    return if heroes.empty?
    @returners -= heroes
    @reserves += heroes
    KRumors.refresh = true
  ...
  def add_actor(actor_id)
    ...
    if @actors.size < KRumorSetup::PARTY_MAX and !@actors.include?(actor)
      @actors << actor
      return $game_player.refresh
    end
    return if groups.include?(actor)
    @reserves << actor unless @non_gossipers.include?(actor_id)
  end

  def swap_actor_reserve(actor_id, reserve_id)
    aids = @actors.map{|a| a.id }
    return unless (apos = aids.index(actor_id))
    rids = @reserves.map{|r| r.id }
    return unless (rpos = rids.index(reserve_id))
    @actors[apos], @reserves[rpos] = @reserves[rpos], @actors[apos]
    return true
  end
  def swap_actors(m, n) @actors[m], @actors[n] = @actors[n], @actors[m] end
  def reserve?(actor_id) @reserves.include?($game_actors[actor_id]) end
  def traveler?(actor_id) @travelers.include?($game_actors[actor_id]) end
  ...
  def remove_reserve(actor_id) @reserves.delete($game_actors[actor_id]) end
...
class RumorMenuBackdrop < Sprite
  def initialize(new_x, new_y, w, max)
    ...
    self.y = new_y
    h = (max + 1) * 24 + 20
    bit = RPG::Cache.picture(KRumorSetup::LIST_BACKDROP)
...
class RumorListTitle < Sprite
  def initialize(new_x, new_y, new_z, label)
    ...
    self.y = new_y
    self.z = new_z
    self.bitmap = b = RPG::Cache.picture(KRumorSetup::LIST_TITLE_ICON).dup
...
class RumorInfoSprite < Sprite
  ...
  def show_quests_found
    ...
    @rumor.quest_ids.each do |qid|
      if Kuests[qid]
        bit = RPG::Cache.icon(KuestSetup::STAR_ICONS[qid - 1])
...
class RumorGroupCursor < RPG::Sprite
  def initialize(new_x, height, mode, new_limit, view)
    super(view)
    if mode == :hero
      self.bitmap = RPG::Cache.picture(KRumorSetup::HERO_CURSOR)
      @base_y = height - bitmap.height
    else
      self.bitmap = RPG::Cache.icon(KRumorSetup::LOCATE_CURSOR)
      @base_y = 0
    end
    @cursor_se = $data_system.cursor_se
    @limit = new_limit
    @index = 0
    @change_y = false
    self.x = new_x
...
class HorizonCursor < Sprite
  def initialize(new_x, new_y, max, offset, view)
    ...
    self.y = new_y
    self.z = 350
    self.bitmap = RPG::Cache.icon(KRumorSetup::CANCEL_ICON)
    @cursor_se = $data_system.cursor_se
    @max = max
    @index = 0
  end
...
class HeroDataSprite < Sprite
  ...
  def initialize(pos, view)
    ...
    reset_y
    self.z = 5
    self.bitmap = Bitmap.new(460, 100)
    @hero = KRumors.heroes[pos]
    refresh
  end
  ...
  def draw_labels
    bitmap.draw_text(136, 0, 52, 25, HERO_LVL)
    bitmap.draw_text(240, 0, 128, 25, HERO_NEEDS)
    bitmap.draw_text(0, 25, 100, 25, STATUS)
    bitmap.draw_text(224, 25, 128, 25, RUMORS_TOTAL)
  end

  def draw_basic_data
    bitmap.draw_text(0, 0, 128, 25, @hero.name)
    bitmap.draw_text(172, 0, 60, 25, @hero.level.to_s, 2)
    bitmap.draw_text(328, 0, 100, 25, @hero.next_rest_exp_s, 2)
    bitmap.draw_text(368, 25, 60, 25, @hero.rumor_total.to_s, 2)
  end

  def draw_status
    bitmap.draw_text(0, 25, 184, 25, STATUSES[!@hero.hplow?], 2)
  end
...
class TravelDataSprite < HeroDataSprite
  def refresh
    ...
    set_group_font_colors
    draw_labels
    bitmap.draw_text(0, 50, 128, 25, DESTINATION)
    set_group_data_font_colors
    draw_basic_data
    bitmap.draw_text(136, 50, 280, 25, location)
    draw_status
  end
end
...
class RumorDataSprite < HeroDataSprite
  def refresh
    ...
    set_group_font_colors
    draw_labels
    bitmap.draw_text(0, 50, 128, 25, HOT_RUMORS)
    bitmap.draw_text(200, 50, 128, 25, OLD_RUMORS)
    set_group_data_font_colors
    draw_basic_data
    bitmap.draw_text(128, 50, 60, 25, total.to_s, 2)
    bitmap.draw_text(328, 50, 60, 25, old.to_s, 2)
    draw_status
  end
end
...
class RumorSpriteset
  ...
  def make_backdrop_main_menu
    ...
    @backdrop.bitmap = RPG::Cache.title(BACKDROP + @width.to_s)
  ...
  def make_titles_lists
    ...
    @sprites << @rumor_info = RumorInfoSprite.new
    @rumor_info.visible = false
  end

  def make_cursors
    @sprites << @cursor = RPG::Sprite.new
    @cursor.x = @options_sprite.x - 28
    @cursor.y = 158
    @cursor.blink_on
    @cursor.bitmap = RPG::Cache.icon(STAGE_CURSOR)
  ...
  def make_rumor_backdrops
    drop = RPG::Cache.title(RUMOR_BACKDROP + @width.to_s)
    half = @width / 2
    2.times do |n|
    ...
    @warning_sprite.bitmap = b = RPG::Cache.picture(WARNING_BACKDROP)
    @warn_width = b.width
  ...
  def make_cancel_sprites
    ...
    @cancel_sprite.bitmap = bit = RPG::Cache.picture(CANCEL_BACKDROP)
    bw = bit.width
    @cancel_sprite.x = cw = (@width - bw) / 2
    @cancel_sprite.y = ch = (@height == 480 ? 100 : 180)
    @cancel_sprite.z = 300
    bit.font.size = 36
  ...
  def collect_heroes_data
    @face_names = KRumors.heroes.map{|h| h.character_name }
  ...
  def draw_face(pos)
    ...
    face.bitmap = RPG::Cache.face(@face_names[pos])
  end

  def refresh_rumor_data
    @all_rumors = KRumors.heroes.map{|h| h.rumors }
    @face_max.times do |n|
      ...
      unless @failures[n]
        ...
        sprite.bitmap = RPG::Cache.picture(FAILED_PICTURE)
      end
      rumors = @all_rumors[n]
      void = rumors.empty?
  ...
  def start_location_stage
    ...
    @group_cursor.blink_off
    @list_cursor.blink_on
    toggle_location_sprites(true)
  end

  def turn_off_location_sprites
    toggle_location_sprites(false)
    @list_cursor.blink_off
  end
  ...
  def locate_to_group_stage
    @group_cursor.blink_on
    turn_off_location_sprites
  ...
  def open_scroll
    @rumor_backdrops[0].x -= 10
    @rumor_backdrops[1].x += 10
...
class KRumorScene
  def main
    ...
    @ok_se = $data_system.decision_se
    @cancel = $data_system.cancel_se
  ...
  def update_main
    ...
    if Input.trigger?(Input::B)
      ...
      if pos < 3
        ...
        return $game_system.se_play(@buzzer) if KRumors.heroes.empty?
        $game_system.se_play(@ok_se)
        @spriteset.start_group_stage
        @heroes_max = KRumors.heroes.size
        KRumors.stop_refresh = true
        return @stage = pos == 1 ? :travels : :group
      end
      return $game_system.se_play(@buzzer) if $game_party.books.empty?
      $game_system.se_play(@ok_se)
      @spriteset.start_group_stage
      @stage = :book_list
    end
  ...
  def update_group
    ...
    if Input.trigger?(Input::B)
      ...
      return $game_system.se_play(@buzzer) if KRumors.cannot_send?
      $game_system.se_play(@ok_se)
      if KRumors.index == 0
  ...
  def update_read_rumor
    $game_party.update_returners
    if Input.trigger?(Input::LEFT)
      return $game_system.se_play(@buzzer) if KRumors.not_enough_pages
      $game_system.se_play(@cursor)
...
class Scene_Menu
  ...
  def update
    $game_system.update
    kyon_krumors_scn_menu_up
  end
end

class Scene_Load
  alias kyon_krumors_scn_load_on_dec on_decision
  def on_decision(filename)
    kyon_krumors_scn_load_on_dec(filename)
