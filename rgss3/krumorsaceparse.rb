...
# You may need to change the OPINION_LINES_MAX values if deemed necessary.

module KRumorSetup
  OPINION_LINES_MAX = Graphics.width == 544 ? 3 : 4
end
...
module KRumors
  def self.parse_locations
    ...
    filename = 'Data/KRumorsLocations.rvdata2'
  ...
  def self.parse_rumors
    ...
    File.open('Data/KRumorsNPC.rvdata2','wb'){|file| Marshal.dump(@npc_rumors, file) }
  end
  ...
  def self.parse_rumor_opinions
    ...
    total = KRumorSetup::OPINION_LINES_MAX
    opinions = {}
    filenames.each do |filename|
      ...
      lines.shift
      generic = []
      total.times { generic << lines.shift.chomp }
      lines.shift
      opinion.default = generic
      (lines.size / 6).times do |n|
        hero_id = lines.shift.scan(/\d+/)[0].to_i
        opinion[hero_id] = []
        total.times { opinion[hero_id] << lines.shift.chomp }
        lines.shift
      end
    end
    name = 'Data/KRumorsOpinions.rvdata2'
  ...
  def self.parse_book_titles
    ...
    name = 'Data/KRumorsBookTitles.rvdata2'
